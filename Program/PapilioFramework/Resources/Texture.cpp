...
*         互換性のあるフォーマットが見つからない場合はGUID_WICPixelFormatDontCareを返す.
*/
...
/**
* DXGIフォーマットから１ピクセルバイトの数を得る
* @param dxgiFormat DXGIフォーマット
* @return dxgiFormatに対応するバイト数
*/
...
* 作成したテクスチャに関するデータはtexture1に格納される
* @pre Begin(),End()の間で呼ぶ
...
* @param name テクスチャ用リソースに付ける名前（デバッグ用）名前が不要な場合はnullptrを指定する
* @retval true 作成成功
* @retval false　作成失敗
*/
bool TextureLoader::Create(Texture& texture,
	int index,
	...
	const wchar_t* name)
{
	...
	//デフォルトヒープにデータを転送するためにアップロードヒープを作成
	UINT64 textureHeapSize;
	device->GetCopyableFootprints(
		&desc, 0, 1, 0, nullptr, nullptr, nullptr, &textureHeapSize);
	...
	if (UpdateSubresources<1>(commandList.Get(), textureBuffer.Get(),
		uploadBuffer.Get(), 0, 0, 1, &subresource) == 0)
	{
		return false;
	}
	commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(textureBuffer.Get(),
	...
	//テクスチャオブジェクトにテクスチャ情報を設定
	texture.resource = textureBuffer;
	texture.format = desc.Format;
	texture.handle = CD3DX12_GPU_DESCRIPTOR_HANDLE(descriptorHeap->GetGPUDescriptorHandleForHeapStart(),
		index,
		descriptorSize);
...
bool TextureLoader::LoadFromFile(Texture& texture, int index, const wchar_t* filename)
{
	...
	if (imageConverted)
	{
		if (FAILED(converter->CopyPixels(
			nullptr, bytesPerRow, imageSize, imageData.data())))
		{
			return false;
		}
	}
